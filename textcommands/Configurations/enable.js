module.exports = {
  name: "enable",
  description: ["Kích hoạt một lệnh - một mục lệnh hoặc tất cả lệnh trong kênh.","Enable one, a category or all commands."],
  aliases: ["en", "ec", "enablecommands", "allow"],
  usage:["{prefix}enable <commands>", "{prefix}enable <commands>"],
  cooldown: 0,
	category: "Configs",
	canuse: "g-admins",
  errorcd : ["{time}", "{time}"],
...
const db = new QuickDB({table: "DB"})
    ...
    if (args[0] == `all`) {
      ...
      let messages = [
        `${client.e.success} | Đã kích hoạt tất cả lệnh trong kênh!`,
        `${client.e.success} | Enabled All the Commands in this Channel!!`
      ]
      ...
      let messagess = [
        ...
        `${done} | Command(s) \`${commandsa}\` has/have been **Enabled** at <#${message.channel.id}>!`
      ]
