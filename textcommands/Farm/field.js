...
module.exports = {
  name: "field",
  description: ["Xem trang trại của bạn!", "Check your fields"],
  aliases: ['ruong', 'r', 'f'],
  usage: ["{prefix}ruong", "{prefix}field"],
  cooldown: 15000,
  category: "Farm",
  canuse: "everyone",
...
\`pig\` 𝑷𝒊𝒈 ${tnheo}`)
    ...
    let a = await message.reply(`${lang == "vi"
      ? `${client.e.load} | Xin đợi một chút!`
      : `${client.e.load} | Please wait a sec...!`}`)
    let abc = await a.edit({ content: "", embeds: [fieldembed1] }).catch(e => console.log(e))
    let msgw = await chuyen_trang(client, abc, author, embeds)
    await thu_hoach(client,
      client.hg.ot, client.hg.lua, client.hg.dautay, client.hg.ngo, client.hg.cachua, client.hg.dao, client.hg.khoaimi, client.hg.mia, client.hg.khoaitay, client.hg.duagang, client.hg.carot, client.hg.caingot, client.hg.mit,
      abc, message.author.id)
  }
}
...
async function chuyen_trang(client, message, authorid, embeds) {
  ...
  var collector = queueEmbed.createMessageComponentCollector({
    filter: interaction => (interaction.isButton() || interaction.isStringSelectMenu()) && interaction.message.author.id == client.user.id,
  })
  collector.on("collect", async (interaction) => {
    ...
    if (interaction.customId == "next-page") {
      if (currentPage < embeds.length - 1) {
        currentPage++;
        await queueEmbed.edit({ content: `**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]], components: [buttonrow1] });
      }
      else {
        currentPage = 0
        await queueEmbed.edit({ content: `**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]], components: [buttonrow1] });
     ...
     await queueEmbed.edit({ content: `**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]], components: [buttonrow1] });
      ...
      await interaction.message.edit({ embeds: [embeds[0]], components: [buttonrow1] })
    }
  })
}
...
async function checktimethu(message, client, hatgiong, name, author, timeout) {
  ...
  let timeheo = await client.checkcd(lastfeedheo, timeout)
...
async function thu_hoach(client, hg1, hg2, hg3, hg4, hg5, hg6, hg7, hg8, hg9, hg10, hg11, hg12, hg13, message, authorid) {
  ...
  const filter = interaction => interaction.isButton()
    ...
    const collector = message.channel.createMessageComponentCollector({ filter: filter, time: 100000 });
  collector.on("collect", async (interaction) => {
    let hgs = {
      "ot": hg1,
      "lua": hg2,
...
function checkelite(name) {
  let tinhhoa = 0
  if (name == `ot`) tinhhoa = 1
  else if (name == `lua`) tinhhoa = 1
  else if (name == `dautay`) tinhhoa = 3
  else if (name == `ngo`) tinhhoa = 3
  else if (name == `cachua`) tinhhoa = 5
  else if (name == `dao`) tinhhoa = 5
  else if (name == `khoaimi`) tinhhoa = 7
  else if (name == `mia`) tinhhoa = 7
  else if (name == `khoaitay`) tinhhoa = 12
  else if (name == `duagang`) tinhhoa = 12
  else if (name == `carot`) tinhhoa = 17
  else if (name == `caingot`) tinhhoa = 17
  else if (name == `mit`) tinhhoa = 35
  return tinhhoa
}
