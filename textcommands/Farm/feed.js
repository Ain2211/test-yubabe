module.exports = {
  name: "feed",
  description: ["Cho thú nuôi ăn :D", "Feed the animals"],
  aliases: ["n", "choan", "nuoi"],
  usage: ["{prefix}nuoi", "{prefix}feed"],
  cooldown: 15000,
  category: "Farm",
  canuse: "everyone",
...
async function nuoi_thu(client, hg1, hg2, hg3, n1, n2, n3, message, authorid, username) {
  ...
  collector.on("collect", async (interaction) => {
    ...
    if (interaction.customId == n1) {
      ...
      const ErrorFeed = [
        `${client.e.fail} | **${username}** , bạn đã hết thóc! Mua thim ik <3`,
        `${client.e.fail} | **${username}** , you've ran out of Chicken's paddy! Please buy some! <3`
      ]
      if (thoc < 1) return await client.send(client, message, ErrorFeed, null).catch(e => console.log(e))
      const ErrorChicken = [
        `${client.e.fail} | **${username}** , bạn không còn gà để nuôi, mua gà ik <3`,
        `${client.e.fail} | **${username}** , you have no chicken, please buy one to raise it! <3`
      ]
      ...
      if (!cooldown) {
        const DelayMSG = [
          `${client.e.fail} | **${message.author.username}**, bạn phải chờ : \`${used.h + `:` + used.m + `:` + used.s}s\` để cho gà ăn tiếp!`,
          `${client.e.fail} | **${message.author.username}**, you have to wait : \`${used.h + `:` + used.m + `:` + used.s}s\` to feed those Chickens!`
        ]
      ...
      await interaction.deferUpdate()
      const ErrorFeed = [
        `${client.e.fail} | **${username}** , bạn đã hết cỏ! Mua thim ik <3`,
        `${client.e.fail} | **${username}** , you've ran out of Cow's grass! Please buy some! <3`
      ]
      if (co < 1) return await client.send(client, message, ErrorFeed, null).catch(e => console.log(e))
      const ErrorCow = [
        `${client.e.fail} | **${username}** , bạn không còn bò để nuôi, mua bò ik <3`,
        `${client.e.fail} | **${username}** , you have no cow, please buy one to raise it! <3`
      ]
      ...
      if (!cooldown) {
        const DelayMSG = [
          `${client.e.fail} | **${message.author.username}**, bạn phải chờ : \`${used.h + `:` + used.m + `:` + used.s}s\` để cho bò ăn tiếp!`,
          `${client.e.fail} | **${message.author.username}**, you have to wait : \`${used.h + `:` + used.m + `:` + used.s}s\` to feed those Cows!`
        ]
        const delay = await client.send(client, message, DelayMSG, null).catch(e => console.log(e))
        await client.sleep(5000)
        await delay.delete()
      ...
      await interaction.deferUpdate()
      const ErrorFeed = [
        `${client.e.fail} | **${username}** , bạn đã hết cám! Mua thim ik <3`,
        `${client.e.fail} | **${username}** , you've ran out of Pig's pran! Please buy some! <3`
      ]
      if (camheo < 1) return await client.send(client, message, ErrorFeed, null).catch(e => console.log(e))
      const ErrorPig = [
        `${client.e.fail} | **${username}** , bạn không còn heo để nuôi, mua heo ik <3`,
        `${client.e.fail} | **${username}** , you have no pig, please buy one to raise it! <3`
      ]
      ...
      if (!cooldown) {
        const DelayMSG = [
          `${client.e.fail} | **${message.author.username}**, bạn phải chờ : \`${used.h + `:` + used.m + `:` + used.s}s\` để cho heo ăn tiếp!`,
          `${client.e.fail} | **${message.author.username}**, you have to wait : \`${used.h + `:` + used.m + `:` + used.s}s\` to feed those Pigs!`
        ]
        const delay = await client.send(client, message, DelayMSG, null).catch(e => console.log(e))
        await client.sleep(5000)
        await delay.delete()
