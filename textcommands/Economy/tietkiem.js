...
module.exports = {
  name: "deposit",
  description: ["Gửi tiền của bạn vào ngân hàng!", "Deposit your money to your bank account!"],
  aliases: ["dep", "guitietkiem", "tietkiem", "tk"],
  usage: ["{prefix}tietkiem <soluong>", "{prefix}dep <amount>"],
  cooldown: 0,
  category: "Eco",
  canuse: "everyone",
  errorcd: ["Từ từ nào! Hãy đợi **{time}** và thử lại", "Please wait for **{time}** and try again!"],
  run: async (client, message, args) => {
    ...
    if (!cooldown) {
      const DelayMSG = [
        ...
        `${client.e.fail} | **${message.author.username}**, please check your DMS to see the money! If your DMS not open, please text me at least 1...`
      ]
      ...
      await client.dms(client, message, message.author, Success, null, null).catch(async e => {
        console.log(e)
        await client.send(client, message, Success, null)
      })
    }
  }
}