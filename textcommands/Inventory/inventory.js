...
module.exports = {
  name: "inventory",
  description: ["Xem kho trang bị của bạn", "Check your inventory"],
  aliases: ["inv", 'kho'],
  usage: ["{prefix}inv", "{prefix}inv"],
  cooldown: 30000,
  category: "Inven",
  canuse: "everyone",
  errorcd: ["Bạn vừa mới check inventory ! Xin hãy đợi **{time}**", "Please wait for **{time}** and check your inventory again!"],
  run: async (client, message, args) => {
    const { QuickDB } = require("quick.db")
    const db = new QuickDB({ table: "DB" })
    const lang = await db.get(`${message.guild.id}_languages`)
    let a = await message.channel.send({ content: `${lang == "vi" ? `${client.e.load} | Xin đợi một chút...` : `${client.e.load} | Please wait a sec...`}` }).catch(e => console.log(e))
    await chuyen_trang_Inventory(client, message, a, message.author.id, args, lang).catch(e => console.log(e))
  }
}
...
async function chuyen_trang_Inventory(client, message1, message, authorid, args, lang) {
  let embeds = await generateEmbeds(client, message1, args, lang)
  let currentPage = 0;
...
async function generateEmbeds(client, message, args, lang) {
  if (lang == "vi") {
    let msgnhan = []
    let msgpassport = []
    let ringid = [
...
${msgpassport}`
    ...
    const ppembed = new EmbedBuilder()
      ...
      .setFooter({ text: "Cảm ơn bạn đã bên cạnh Yubabe", iconURL: `https://media.discordapp.net/attachments/978011752610557972/983700981961343026/919967569287446568.gif` })
      .setTimestamp()
    return embeds = [ringembed, gemembed, ppembed]
  }
  else if (lang == "en") {
    let msgnhan = []
    let msgpassport = []
    ...
    const nhanembed = `<a:yl_capoquay:1107968194800525322> **Properties of ${message.author.username}** <a:yl_capoquay:1107968194800525322>
<a:emoji_131:910626774638010378> Ring :
${msgnhan}`
    const passportembed = `<a:yl_capoquay:1107968194800525322> **Properties of ${message.author.username}** <a:yl_capoquay:1107968194800525322>
<a:emoji_131:910626774638010378> Passport :
${msgpassport}`
    ...
    return embeds = [ringembed, gemembed, heroesembed, stoneembed, itemsembed, ppembed]
