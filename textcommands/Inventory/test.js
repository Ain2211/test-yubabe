...
module.exports = {
  name: "sellfish",
  description: ["Bán cá", "sell the fish"],
  aliases: ["banca", "sf"],
  usage: ["{prefix}sf <name> <quanlity>", "{prefix}sf <name> <quanlity>"],
  cooldown: 1000,
  category: "Inven",
  canuse: "everyone",
  errorcd: ["{time}", "{time}"],
  run: async (client, message, args) => {
    ...
    if (args.length <= 2) {
      if (args[0] !== `all`) {
        ...
        await message.reply(`${client.e.success} | **__${message.author.username}__** đã bán **${quanlity}** ${allName} và thu được **__${parseInt(money).toLocaleString("en-us")}__** Ycoin`)
      }
      else if (args[0] == `all`) {
        ...
        await client.cong(authorid, money)
        await message.reply(`${client.e.success} | **__${message.author.username}__** đã bán ${allName} và thu được **__${parseInt(money).toLocaleString("en-us")}__** Ycoin`)
      }
    }
    else {
