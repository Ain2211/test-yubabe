...
module.exports = {
	name: "buy",
	description: ["Mua vật phẩm trong Shop", "Buy items from Shop!"],
	aliases: ["mua"],
	...
	run: async (client, message, args) => {
		...
		if (purchase == '002' || purchase == 'nhanb' || purchase == 'nhanbac') {
			...
			const success = [
				...
				`<:Yu_nhanbac:941435162728730675> | **${member.username}**, you bought **Silver Ring** with **5000,000 Ycoin!**`
			]
			...
			client.tru(member.id, 800 * soluong)
			await client.addgrow(`${message.author.id}`, `thoc`, soluong, 'food')
			const success = [
				`<:Yu_Ycoin:953323682246316082> | **${member.username}**, bạn đã mua <:Yu_thoc:953407482884161566> **${soluong}** <:Yu_thoc:953407482884161566> Bao Thóc với giá **${parseInt(800 * soluong).toLocaleString('En-us')} Ycoin**!`,
				`<:Yu_Ycoin:953323682246316082> | **${member.username}**, you bought <:Yu_thoc:953407482884161566> **${soluong}** <:Yu_thoc:953407482884161566> Chicken's paddy for **${parseInt(800 * soluong).toLocaleString('En-us')} Ycoin**!`
			]
			await client.send(client, message, success, null).catch(e => console.log(e))
		}
			...
			const success = [
				`<:Yu_Ycoin:953323682246316082> | **${member.username}**, bạn đã mua <:Yu_co:953408530474475520> **${soluong}** <:Yu_co:953408530474475520> Bao Cỏ với giá **${parseInt(800 * soluong).toLocaleString('En-us')} Ycoin**!`,
				`<:Yu_Ycoin:953323682246316082> | **${member.username}**, you bought <:Yu_co:953408530474475520> **${soluong}** <:Yu_co:953408530474475520> Cow's Grass for **${parseInt(800 * soluong).toLocaleString('En-us')} Ycoin**!`
			]
			...
			const success = [
				`<:Yu_Ycoin:953323682246316082> | **${member.username}**, bạn đã mua <:Yu_camheo:953407482955436062> **${soluong}** <:Yu_camheo:953407482955436062> Bao Cám Heo với giá **${parseInt(800 * soluong).toLocaleString('En-us')} Ycoin**!`,
				`<:Yu_Ycoin:953323682246316082> | **${member.username}**, you bought <:Yu_camheo:953407482955436062> **${soluong}** <:Yu_camheo:953407482955436062> Pig's pran for **${parseInt(800 * soluong).toLocaleString('En-us')} Ycoin**!`
			]
			...
			if (cash < 7000) return await client.send(client, message, noMoney, null).catch(e => console.log(e))
			await client.addgrow(`${member.id}`, `${tn1}`, 8, 'thu')
			await client.tru(member.id, 7000)
			const success = [
			...
			if (cash < 11000) return await client.send(client, message, noMoney, null).catch(e => console.log(e))
			await client.addgrow(`${member.id}`, `${tn2}`, 10, 'thu')
			await client.tru(member.id, 11000)
			const success = [
			...
			if (cash < 15000) return await client.send(client, message, noMoney, null).catch(e => console.log(e))
			await client.addgrow(`${member.id}`, `${tn3}`, 12, 'thu')
			await client.tru(member.id, 15000)
			const success = [
...
function checktienmp(m) {
	...
	let result
	if (m == arrmypham1["m1"]) result = 7500
	else if (m == arrmypham1["m2"]) result = 7500
	else if (m == arrmypham1["m3"]) result = 56000
	else if (m == arrmypham1["m4"]) result = 126000
	else if (m == arrmypham1["m5"]) result = 41000
	else if (m == arrmypham1["m6"]) result = 91000
	else if (m == arrmypham1["m7"]) result = 56000
	else if (m == arrmypham1["m8"]) result = 500000
	return result
}
/*
