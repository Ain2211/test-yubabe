...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (id == `gb` || id == `gembox` || id == `29`) {
      ...
      if (gembox < 1) return await client.send(client, message, lackGemboxes2, null)
      let soluong = parseInt(args[1])
      const errorAmount = [
        `:x: | **${message.author.username} Bạn không thể nhập số âm !**`,
        `:x: | **${message.author.username} You can't insert Negatives !**`
      ]
      ...
      if (gembox < soluong) return await client.send(client, message, lackGemboxes, null)
      ...
      const LackOfBox = [
        `Bạn không còn hộp ngọc PRO để dùng.`,
        `You don't have enough PRO GEMBOX to use.`
      ]
      if (gembox < 1) return await client.send(client, message, LackOfBox, null)
      let soluong = parseInt(args[1])
      const errrorOfAmoiint = [
        `:x: | **${message.author.username} Bạn không thể nhập số âm !**`,
        `:x: | **${message.author.username} Amount can't be negative !**`
      ]
      if (soluong < 0) return await client.send(client, message, errrorOfAmoiint, null)
      if (!soluong && gembox > 0) soluong = 1
      if (args[1] == `all`) soluong = gembox
      if (soluong > 20) soluong = 20
      const LackofSth12 = [
        `:x: | **${message.author.username}**, bạn không có đủ hộp ngọc PRO để dùng!`,
        `:x: | **${message.author.username}**, you don't have enough PRO gemboxes to use!!`
      ]
      ...
      const LackOfBox = [
        `Bạn không còn hộp ngọc VIP để dùng.`,
        `You don't have enough VIP GEMBOX to use.`
      ]
      if (gembox < 1) return await client.send(client, message, LackOfBox, null)
      let soluong = parseInt(args[1])
      const errrorOfAmoiint = [
      ...
      if (soluong > 20) soluong = 20
      const LackofSth12 = [
        `:x: | **${message.author.username}**, bạn không có đủ hộp ngọc VIP để dùng!`,
        `:x: | **${message.author.username}**, you don't have enough VIP gemboxes to use!!`
      ]
      if (gembox < soluong) return client.send(client, message, LackofSth12, null)
      ...
      const gemlIsZero = [
        `:x: | **${message.author.username}**, bạn không có ngọc ${idngoc1}`,
        `:x: | **${message.author.username}**, you don't have ${idngoc1}`
      ]
      if (geml == 0) return await client.send(client, message, gemlIsZero, null)
      const gemlIsZero2 = [
        `:x: | **${message.author.username}**, bạn không thể dùng chung loại ngọc!`,
        `:x: | **${message.author.username}**, you can't use same type of game!`
      ]
      if (buff1 > 0) return await client.send(client, message, gemlIsZero2, null)
      const gemlIsZero23 = [
        `:x: | **${message.author.username}**, bạn không thể dùng thêm ngọc khi đã dùng KINGSTONE!`,
        `:x: | **${message.author.username}**, you can't use more gems while using KINGSTONE!`
      ]
      if (buff4 > 0) return await client.send(client, message, gemlIsZero23, null)
      ...
      const gemlIsZero = [
        `:x: | **${message.author.username}**, bạn không có ngọc ${idngoc2}`,
        `:x: | **${message.author.username}**, you don't have ${idngoc2}`
      ]
      if (geml == 0) return await client.send(client, message, gemlIsZero, null)
      const gemlIsZero2 = [
        `:x: | **${message.author.username}**, bạn không thể dùng chung loại ngọc!`,
      ...
      if (buff2 > 0) return await client.send(client, message, gemlIsZero2, null)
      const gemlIsZero23 = [
        `:x: | **${message.author.username}**, bạn không thể dùng thêm ngọc khi đã dùng KINGSTONE!`,
        `:x: | **${message.author.username}**, you can't use more gems while using KINGSTONE!`
      ]
      ...
      const gemlIsZero = [
        `:x: | **${message.author.username}**, bạn không có ngọc ${idngoc3}`,
        `:x: | **${message.author.username}**, you don't have ${idngoc3}`
      ]
      if (geml == 0) return await client.send(client, message, gemlIsZero, null)
      const gemlIsZero2 = [
        `:x: | **${message.author.username}**, bạn không thể dùng chung loại ngọc!`,
      ...
      if (buff3 > 0) return await client.send(client, message, gemlIsZero2, null)
      const gemlIsZero23 = [
        `:x: | **${message.author.username}**, bạn không thể dùng thêm ngọc khi đã dùng KINGSTONE!`,
        `:x: | **${message.author.username}**, you can't use more gems while using KINGSTONE!`
      ]
      ...
      const lackk1 = [
        `:x: | **${message.author.username}**, bạn không có ngọc ${idngoc4}`,
        `:x: | **${message.author.username}**, you don't have ${idngoc4}`
      ]
      if (gem < 1) return await client.send(client, message, lackk1, null)
      const lackk2 = [
        `:x: | **${message.author.username}**, bạn không thể dùng thêm  KINGSTONE khi vẫn còn sức mạnh đá quý!`,
        `:x: | **${message.author.username}**, you can't use KINGSTONE while still have another gem power!`
      ]
      if (buff1 > 0) await client.send(client, message, lackk2, null)
      if (buff2 > 0) await client.send(client, message, lackk2, null)
      if (buff3 > 0) await client.send(client, message, lackk2, null)
      if (buff4 > 0) await client.send(client, message, lackk2, null)
      ...
      let userNotExist = [
        `:x: | Bạn chưa tạo nhân vật, gõ Yme để tạo!`,
        `:x: | You haven't created Character, type Yme !`
      ]
      if (!userProfile) return await client.reply(client, message, userNotExist, null)
      
      let age = userProfile.me.age
      let userNotAge = [
        `:x: | Bạn chưa đủ tuổi để sử dụng mỹ phẩm, còn quá nhỏ mà :( \`ít nhất 7 tuổi\`!`,
        `:x: | You're too young to use cosmetics, try again after 7 years old !`
      ]
      if (age < 7) return await client.reply(client, message, userNotAge, null)
      let amount = await client.item(message.author.id, null, idmp, null, "mypham")
      let notEnough = [
        `:x: | Bạn không còn đủ ${idmp} để dùng!`,
        `:x: | You don't have enough ${idmp} to use!`
      ]
      if (amount < 1) return await client.reply(client, message, notEnough, null)
