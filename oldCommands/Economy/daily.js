...
module.exports = {
  ...
  description2: 'Claim your daily rewards',
  usage2: "Ydaily",
  usage: `Ydaily`,
  ...
  run: async (client, message, args) => {
    ...
    if (!cooldown) {
      ...
      let a = await client.cd(message.author.id, `daily6`)
      ...
      if (!after) {
        await client.timeout(message.author.id, `daily6`)
        let errorDaily = [
        ...
        if (!pro && !vip) {
          ...
          const messageToSend = [
            `${msg[0]}
${luckyicon} | **${user.username}**, bạn đã được tặng 3 <:GEMBOX:982028743952441355>${profile ? msg1[0] : ` `} cho ngày hôm nay!`,
            `${msg[1]}
${luckyicon} | **${user.username}**, you have been awarded 3 <:GEMBOX:982028743952441355>${profile ? msg1[1] : ` `} for today!`
          ]
          ...
          let msg1 = []
          let soluong = 3
          if (profile) soluong += 2, msg1[0] = `, và 2 hộp thêm vì đã cưới <@${profile.husbandid}> `, msg1[1] = `, and 2 bonuses for married <@${profile.husbandid}> `
          await client.addgem(user.id, `<:PRO_GEMBOX:982028744057298964>`, 2, 0)
          await client.addgem(user.id, `<:GEMBOX:982028743952441355>`, soluong, 0)
          const messageToSend = [
            `${msg[0]}
<:ProPassport:988093838348410930> | **${user.username}**, bạn đã được tặng 3 <:GEMBOX:982028743952441355>${profile ? msg1[0] : ` `} và được thêm 2 <:PRO_GEMBOX:982028744057298964> cho ngày hôm nay vì đã đăng ký PRO-PASSPORT !`,
            `${msg[1]}
<:ProPassport:988093838348410930> | **${user.username}**, and you've been award 3 <:GEMBOX:982028743952441355>${profile ? msg1[1] : ` `} and bonuses 2 <:PRO_GEMBOX:982028744057298964> for today due to your PRO-PASSPORT Subcription!`
          ]
          await client.send(client, message, messageToSend, null).catch((e) => console.log(e))
        }
          ...
          let msg1 = []
          let soluong = 3
          if (profile) soluong += 3, msg1[0] = `, và 3 hộp thêm vì đã cưới <@${profile.husbandid}> `, msg1[1] = `, and 3 bonuses for married <@${profile.husbandid}> `
          await client.addgem(user.id, `<:PRO_GEMBOX:982028744057298964>`, 2, 0)
          await client.addgem(user.id, `<:VIP_GEMBOX:982028743889543278>`, 2, 0)
          await client.addgem(user.id, `<:GEMBOX:982028743952441355>`, soluong, 0)
          const messageToSend = [
            `${msg[0]}
<:VIPPassport:988093810955411456> | **${user.username}**, bạn đã được tặng 3 <:GEMBOX:982028743952441355>${profile ? msg1[0] : ` `} và được thêm 2 <:PRO_GEMBOX:982028744057298964>, 2 <:VIP_GEMBOX:982028743889543278> cho ngày hôm nay vì đã đăng ký VIP-PASSPORT !`,
            `${msg[1]}
<:VIPPassport:988093810955411456> | **${user.username}**, and you've been award 3 <:GEMBOX:982028743952441355>${profile ? msg1[1] : ` `} and bonuses 2 <:PRO_GEMBOX:982028744057298964>, 2 <:VIP_GEMBOX:982028743889543278> for today due to your VIP-PASSPORT Subcription!`,
          ]
          await client.send(client, message, messageToSend, null).catch((e) => console.log(e))
        }
        ...
        await client.reply(client, message, yearPassBy, null)
        return
      }
      else if (after && !inday) {
        await client.timeout(message.author.id, `daily6`)
...
${luckyicon} | **${user.username}**, bạn đã được tặng 3 <:GEMBOX:982028743952441355>${profile ? msg1[0] : ` `} cho ngày hôm nay!`,
            `${msg[1]}
${luckyicon} | **${user.username}**, you've been awarded 3 <:GEMBOX:982028743952441355>${profile ? msg1[1] : ` `} for today!`
          ]
          await client.send(client, message, messageToSend, null).catch((e) => console.log(e))
        }
        else if (pro) {
          let msg1 = []
          let soluong = 3
          if (profile) soluong += 2, msg1[0] = `, và 2 hộp thêm vì đã cưới <@${profile.husbandid}> `, msg1[0] = `, and 2 bonuses for married <@${profile.husbandid}> `
          await client.addgem(user.id, `<:PRO_GEMBOX:982028744057298964>`, 2, 0)
          await client.addgem(user.id, `<:GEMBOX:982028743952441355>`, soluong, 0)
          const messageToSend = [
            `${msg[0]}
<:ProPassport:988093838348410930> | **${user.username}**, bạn đã được tặng 3 <:GEMBOX:982028743952441355>${profile ? msg1[0] : ` `} và được thêm 2 <:PRO_GEMBOX:982028744057298964> cho ngày hôm nay vì đã đăng ký PRO-PASSPORT !`,
            `${msg[1]}
<:ProPassport:988093838348410930> | **${user.username}**, you have been awarded 3 <:GEMBOX:982028743952441355>${profile ? msg1[1] : ` `} and bonuses 2 <:PRO_GEMBOX:982028744057298964> for today due to your PRO-PASSPORT Subcription !`
          ]
          await client.send(client, message, messageToSend, null).catch((e) => console.log(e))
        }
        else if (vip) {
...
<:VIPPassport:988093810955411456> | **${user.username}**, bạn đã được tặng 3 <:GEMBOX:982028743952441355>${profile ? msg1[0] : ` `} và được thêm 2 <:PRO_GEMBOX:982028744057298964>, 2 <:VIP_GEMBOX:982028743889543278> cho ngày hôm nay vì đã đăng ký VIP-PASSPORT !`,
            `${msg[1]}
<:VIPPassport:988093810955411456> | **${user.username}**, you've been awarded 3 <:GEMBOX:982028743952441355>${profile ? msg1[1] : ` `} and bonuses 2 <:PRO_GEMBOX:982028744057298964>, 2 <:VIP_GEMBOX:982028743889543278> for today due to your VIP-PASSPORT Subcription !`
          ]
          await client.send(client, message, messageToSend, null).catch((e) => console.log(e))
        }
        let userProfile = await lifeSchema.findOne({ authorid: message.author.id })
