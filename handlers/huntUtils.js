...
const animal = require("../config/animal.json")
...
const BatThuThuong = async (client, message) => {
  ...
  let denbu = await db.get(`${message.author.id}.denbumatbuff11`)
  if (!denbu) {
  await db.set(`${message.author.id}.denbumatbuff11`, true)
  await client.addgem(message.author.id,"<:G_gem_04:982028744057294848>",1,4)
  await client.cong(message.author.id, 10000)
  await message.reply("Bạn đã được tặng quà đền bù bao gồm 1 viên KINGSTONE  <:G_gem_04:982028744057294848> và 10,000 Ycoin để khởi nghiệp, chúc bạn may mắn !")
  }
  ...
  if (a + b < limit) {
    ...
    collector.on('end', async collected => {
      if (collected.size > 0) {
        ...
        banned.save()
        await db.set(`${message.author.id}_softban1`, true);
        let messagess = [
...
function chonthu(animals, lucky, pro, vip, heso, double) {
  ...
  if (lucky && !pro && !vip) {//logic khi chỉ dùng gem03
    for (let i = 0; i < hesothat; i++) {
      ...
      let vipRate = null
      let proRate = null
      let devilRate = 0.000599
      let gloryRate = 0.000999
        if (rand < devilRate) {
    ...
    for (let i = 0; i < hesothat; i++) {
      ...
      let vipRate = 0.0000149
      let proRate = 0.0000299
      let devilRate = 0.000599
      let gloryRate = 0.000999
      if (rand < vipRate) {
